...
def collate_fn(batch):
    ...
    feature_list, text_feature_list, gt_timestamps_list, labels, caption_list, gt_raw_timestamp, raw_duration, raw_caption, key = zip(
        *batch)
    
    text_feature_list = [torch.from_numpy(x) for x in text_feature_list]
    ...
    dt = {
        ...
        "cap":
            {   "embed": text_feature_list, # [tensor], [(gt_all_event_num, text_feature_dim)]
...
class PercentageSubsetDataset(Dataset):
    def __init__(self, original_dataset, percentage):
        self.original_dataset = original_dataset
        self.percentage = percentage
        total_samples = len(original_dataset)
        self.subset_size = int(percentage * total_samples)
        self.indices = random.sample(range(total_samples), self.subset_size)
        self.translator = original_dataset.translator
...
class EDVCdataset(Dataset):
    ...
    def __init__(self, anno_file, feature_folder, text_feature_folder, translator_json, is_training, proposal_type, opt):
        ...
        if is_training:
            ...
            self.translator = Translator(translator_json, opt.vocab_size_val)
        ...
        self.text_feature_folder = text_feature_folder
...
class PropSeqDataset(EDVCdataset):
    ...
    def __init__(self, anno_file, feature_folder, text_feature_folder, translator_pickle, is_training, proposal_type,

                 opt):
        super(PropSeqDataset, self).__init__(anno_file,
                                             feature_folder, text_feature_folder, translator_pickle, is_training, proposal_type,
                                             opt)
    ...
    def load_feats(self, key):
        ...
        if type(vf_types) == list:
            assert type(self.feature_folder) == list and len(vf_types) == len(self.feature_folder), f"feature_folder: {type(self.feature_folder)} and len(vf_types): {len(vf_types)}, len(feature_folder): {self.feature_folder}"
            feats_dict = {}
            text_feats_dict = {}
            all_padding = True
            for vf_type, vf_folder, tf_folder in zip(vf_types, self.feature_folder, self.text_feature_folder):
                feats, text_feats, is_padding = get_feats(key, vf_type, vf_folder, tf_folder)
                ...
                feats_dict[vf_type] = feats
                text_feats_dict[vf_type] = text_feats
            if all_padding:
                print('all feature files of video {} do not exist'.format(key))
            out = np.concatenate([feats_dict[type_] for type_ in vf_types], axis=-1)
            out_text = np.concatenate([text_feats_dict[type_] for type_ in vf_types], axis=-1)
        else:
            out, out_text, is_padding = get_feats(key, vf_types, self.feature_folder, self.text_feature_folder, data_norm=self.opt.data_norm)
            if self.opt.data_rescale:
    ...
    def __getitem__(self, idx):
        ...
        # print(idx, random_ids, text_feats.shape, feats.shape, len(captions), len(gt_timestamps))
        try:
            text_feats = text_feats[np.sort(random_ids), :]
        except:
            print('Error in text_feats', text_feats.shape, random_ids)
            # breakpoint()
            text_feats = text_feats[:gt_sample_num, :]
        ...
        return feats, text_feats, gt_featstamps, action_labels, caption_label, gt_timestamps, duration, captions, key
...
def get_feats(key, vf_type, vf_folder, tf_folder, data_norm=False):
    MEAN = VAR = 0
    if vf_type == 'c3d':
        feat_dim = 500
        ...
        feat_dim = 1024
        path = os.path.join(vf_folder, key[2:13] + '.npy')
    elif vf_type == 'UniVL':
        feat_dim = 768
        if ('Tasty' in vf_folder) or ('tasty' in vf_folder):
            path = os.path.join(vf_folder, key[2:] + '.npy')
        elif ('Howto' in vf_folder) or ('howto' in vf_folder):
            path = os.path.join(vf_folder, key + '.npy')
        else:
            path = os.path.join(vf_folder, key[2:13] + '.npy')
        if tf_folder:
            if ('Tasty' in vf_folder) or ('tasty' in vf_folder):
                text_path = os.path.join(tf_folder, key[2:] + '.npy')
            elif ('Howto' in vf_folder) or ('howto' in vf_folder):
                text_path = os.path.join(tf_folder, key + '.npy')
            else:
                text_path = os.path.join(tf_folder, key[2:13] + '.npy')
        else:
            text_path = None
    elif vf_type == 'CLIP-ViP':
        feat_dim = 512
        path = os.path.join(vf_folder, key[2:13] + '.npy')
        if tf_folder:
            text_path = os.path.join(tf_folder, key[2:13] + '.npy')
        ...
        feat_dim = 768
        if 'Anet' in vf_folder or 'anet' in vf_folder:
            path = os.path.join(vf_folder, key[0:13] + '_clip.npy')
        elif 'Tasty' in vf_folder or 'tasty' in vf_folder:
            path = os.path.join(vf_folder, key[2:] + '_clip.npy')
        elif 'howto' in vf_folder or 'vlep' in vf_folder:
            path = os.path.join(vf_folder, key + '_clip.npy')
        else:
            path = os.path.join(vf_folder, key[2:13] + '_clip.npy')
        if tf_folder:
            if 'Tasty' in vf_folder or 'tasty' in vf_folder:
                text_path = os.path.join(tf_folder, key[2:] + '.npy')
            elif 'howto' in vf_folder or 'vlep' in vf_folder:
                ...
                text_path = os.path.join(tf_folder, key[0:13] + '.npy')
        else:
            text_path = None
    elif vf_type == 'resnet':
        feat_dim = 2048
    ...
    if text_path:
        if not os.path.exists(text_path):
            print('Can not find text file {}, padding with zeros'.format(text_path))
            text_feature = np.zeros((5, 768))
        else:
    ...
    return feats, text_feature, padding
